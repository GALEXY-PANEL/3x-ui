...
bash <(curl -Ls https://raw.githubusercontent.com/GALEXY-PANEL/3x-ui/master/install.sh) v3.4.0
```
...
docker run -d \
  ...
  --name 3x-ui \
  ghcr.io/GALEXY-PANEL/3x-ui:latest
```
...
```bash
XUI_NONINTERACTIVE=1 bash <(curl -Ls https://raw.githubusercontent.com/GALEXY-PANEL/3x-ui/master/install.sh)
```
