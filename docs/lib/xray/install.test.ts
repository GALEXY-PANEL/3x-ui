...
describe('buildScriptCommand', () => {
  ...
  it('pins a specific version by passing the tag to master install.sh', () => {
    ...
    expect(cmd).toBe(
      'bash <(curl -Ls https://raw.githubusercontent.com/GALEXY-PANEL/3x-ui/main/install.sh) v3.4.1',
    );
  });
  ...
  it('supports the rolling dev-latest build', () => {
    ...
    expect(cmd).toBe(
      'bash <(curl -Ls https://raw.githubusercontent.com/GALEXY-PANEL/3x-ui/main/install.sh) dev-latest',
    );
  });
});
